...
export default function ArticlePage({ id, content, title, articles }) {
    return (
      <>
        ...
        <Head>
          <title>LPML - For Students - {title}</title>
        </Head>
      </>
    );
}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = getArticles("info/for-students").map(article => `/info/for-students/${article.id}`);
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const articles = getArticles("info/for-students");
